...
class Protein:
    ...
    def __post_init__(self):
        ...
        atom_type_num = (
            ...
            if self.molecular_type == "protein"
            else residue_constants.na_atom_type_num
        )
        ...
        if self.atom_positions.ndim != 3:
            raise ValueError(
                f"expect atom_positions dimension of 3, but got {self.atom_positions.shape}"
            )
        ...
        if self.residue_index.ndim != 1:
            raise ValueError(
                f"expect residue_index dimension of 1, but got {self.residue_index.shape}"
            )
    ...
    def weight(self):
        ...
        atom_types = (
            ...
            if self.molecular_type == "protein"
            else residue_constants.na_atom_types
        )
    ...
    def seq(self, as_seq=False, with_gap=False):
        ...
        restypes_with_x = (
            residue_constants.restypes_with_x
            if self.molecular_type == "protein"
            else residue_constants.nuctypes_with_x
        )
        return_seq_dict = {}
        return_seq = ""
    ...
    def save(self, file):
        from af2_features import save_prot_as_cif, save_prot_as_pdb
...
def from_pdb_string(
    ...
    if len(models) != 1:
        ...
        print(
            f"Only single model PDBs are supported. Found {len(models)} models. Use the 1st model.",
            file=stderr,
        )
    model = models[0]
        ...
        for res in chain:
            ...
            if res.id[2] != " ":
                if insertion_code_process == "insert":
                    print(
                        f"Warning: PDB contains an insertion code at chain {chain.id} and residue index {res.id[1]} -- {res.id}. Insert into chain",
                        file=stderr,
                    )
                    residue_increase_on_chain += 1
                elif insertion_code_process == "ignore":
                    print(
                        f"Warning: PDB contains an insertion code at chain {chain.id} and residue index {res.id[1]} -- {res.id}. Ingored",
                        file=stderr,
                    )
                    continue
                else:
            ...
            for atom in res:
                ...
                res_occupancy[atom_order[atom.name]] = (
                    0 if atom.occupancy is None else atom.occupancy
                )
            if np.sum(mask) < 0.5:
...
def to_pdb(prot: Protein, gap_ter_threshold=600.0) -> str:
    ...
    last_resolved_ca_xyz = (
        atom_positions[0, 1] if atom_mask[0, 1] == 1.0 else None
    )  # Last resolved Ca XYZ
    # Add all atom sites.
    for i in range(aatype.shape[0]):
        # Close the previous chain if in a multichain PDB.
        if last_chain_index != chain_index[i] or (
            last_chain_index == chain_index[i] and last_residue_index > residue_index[i]
        ):
            pdb_lines.append(
                _chain_end(
                    atom_index,
        ...
        #    np.sqrt(np.sum((last_res_ca_xyz - atom_positions[i, 1])**2)) >= gap_ter_threshold:
            ...
            last_resolved_ca_xyz is not None
            and atom_mask[i, 1] == 1.0
            and np.sqrt(np.sum((last_resolved_ca_xyz - atom_positions[i, 1]) ** 2))
            >= gap_ter_threshold
        ):
            pdb_lines.append(
                _chain_end(
