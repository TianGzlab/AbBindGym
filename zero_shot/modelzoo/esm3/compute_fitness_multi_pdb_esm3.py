...
class ESM3Engine:
    """
    Inference Engine for ESM3 multimodal model.
    Only uses the sequence modality for mutation scoring.
    """
    ...
    def __init__(
        ...
        self.model = ESM3.from_pretrained(model_name).to(self.device).eval()

        # ESM3 is a multimodal model with complex internal dtype handling.
        # Using torch.autocast for mixed precision instead of .half()
        # to avoid dtype mismatch errors in internal components.

        # ESM3 uses tokenizers (TokenizerCollection) instead of tokenizer
        # Access the sequence tokenizer via model.tokenizers.sequence
    ...
    def _model_forward_log_probs(self, input_ids_1d: torch.Tensor) -> torch.Tensor:
        """
        Forward pass for ESM3 model (sequence-only).
        ...
        with torch.inference_mode():
            ...
            if self.use_fp16_infer:
                with torch.autocast(device_type="cuda", dtype=torch.float16):
                    ...
                    logits = output.sequence_logits[0].float()  # [T, V]
            else:
                output = self.model(sequence_tokens=input_tensor)
    ...
    def _compute_masked_optimal_batched(
        self, sequence: str, batch_size: int = 8
    ) -> torch.Tensor:
        raw_ids = self.tokenizer.encode(sequence)
        if isinstance(raw_ids, list):
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        help="Model name. E.g., 'esm3_sm_open_v1'",
    )
    parser.add_argument(
        "--input-csv",
        required=True,
    ...
    df[f"esm3_{args.mode}_score"] = final_scores
