...
class VenusPLMEngine:
    """
    High-performance VenusPLM inference engine for DMS scoring.
    ...
    def __init__(
        ...
        *,
        ...
        # Load model
        self.model = TransformerForMaskedLM.from_pretrained(
            model_path,
            config=config,
            local_files_only=True,
        )
        ...
        # Load tokenizer
        self.tokenizer = VPLMTokenizer.from_pretrained(
            model_path,
            local_files_only=True,
        )

        print("X" in self.tokenizer.get_vocab())

        # Move model to device BEFORE converting to FP16
        self.model = self.model.to(self.device)

        # Convert to FP16 after moving to device
    ...
    def get_log_probs(self, sequence: str, mode: str) -> torch.Tensor:
        ...
        if os.environ.get("VPLM_DEBUG_MASKED") == "1":
            unk_id = getattr(self.tokenizer, "unk_token_id", None)
            toks = self.tokenizer(sequence, add_special_tokens=False).input_ids
            print(
                f"[VPLM_DEBUG_MASKED] L={len(sequence)} T_no_special={len(toks)} "
                f"unique={sorted(set(sequence))}",
                flush=True,
            )
            bad = []
            for i, ch in enumerate(sequence):
                ...
                if unk_id is not None and tid == unk_id:
                    bad.append((i, ch))
            print(f"[VPLM_DEBUG_MASKED] unk_chars={bad[:20]} total_unk={len(bad)}", flush=True)
    ...
    def _compute_masked_optimal_batched(
        ...
        if os.environ.get("VPLM_DEBUG_MASKED") == "1":
            try:
                ...
                special_tokens = {"error": str(e)}
            print(
                "[VPLM_DEBUG_MASKED] L=%d T=%d W=%d add_special_tokens=True token0=%s token_last=%s specials=%s"
                % (
                    L,
                    T,
                    W,
                    int(input_ids_full[0].item()) if T > 0 else "NA",
                    int(input_ids_full[-1].item()) if T > 0 else "NA",
                    special_tokens,
                )
            )

        # Residue token indices in [1..L]
...
def score_mutation_delta_logprob(
    ...
    *,
    ...
    Args:
        ...
        tokenizer: VPLMTokenizer used to map AA chars to token ids.
        offset_idx: 1-based offset used in the dataset. Typically 1.
        strict_wt_check: Whether to check WT matches.
...
def main() -> None:
    ...
    parser.add_argument(
        "--model-path",
        default="AI4Protein/VenusPLM-300M",
    ...
    for group in tqdm(processor, desc="Scoring POI groups"):
        ...
        # Skip masking scoring when encounter "X"
        if args.mode == "masked" and "X" in wt_seq:
            for row_idx in group.row_indices:
                final_scores[row_idx] = 0.0
            continue
    ...
    df[f"vplm_{args.mode}_score"] = final_scores
