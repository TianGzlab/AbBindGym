...
# Maximum sequence length for AntiBERTy (512 tokens - 2 special tokens)
ANTIBERTY_MAX_AA_LEN = 510

class AntiBERTyEngine:
    """
    High-performance AntiBERTy inference engine for DMS scoring.

    AntiBERTy processes individual antibody chains separately (not paired sequences).
    For multi-chain antibodies, each chain is scored independently and results are
    concatenated in the proper order.
    ...
    def get_log_probs_per_chain(
        self, chain_sequences: dict, chain_offsets: dict, mode: str
    ) -> torch.Tensor:
        """
        Get per-residue log-probabilities for multiple chains, processing each separately.

        Args:
            chain_sequences: Dict mapping chain IDs to their sequences.
            chain_offsets: Dict mapping chain IDs to their start positions in concatenated sequence.
            mode: "wt" or "masked".

        Returns:
            CPU tensor of shape [L_total, V] in float16, where L_total is sum of all chain lengths.
            Results are ordered according to chain_offsets to match concatenated sequence order.
        """
        if not chain_sequences:
            return torch.empty((0, self.vocab_size), dtype=torch.float16)

        # Sort chains by their offset to maintain proper order
        # Only use chains that exist in both dicts
        valid_chains = [(chain_id, seq) for chain_id, seq in chain_sequences.items()
                        if chain_id in chain_offsets]
        sorted_chains = sorted(valid_chains, key=lambda x: chain_offsets[x[0]])

        # Process each chain separately
        chain_log_probs = []
        for chain_id, sequence in sorted_chains:
            if not sequence:  # Skip empty sequences
                continue
            log_probs = self._get_single_chain_log_probs(sequence, mode)
            chain_log_probs.append(log_probs)

        # Concatenate all chain results
        if not chain_log_probs:
            return torch.empty((0, self.vocab_size), dtype=torch.float16)

        combined_log_probs = torch.cat(chain_log_probs, dim=0)
        return combined_log_probs

    def _get_single_chain_log_probs(self, sequence: str, mode: str) -> torch.Tensor:
        """
        Get (and cache) per-residue log-probabilities for a single chain.
    ...
    def _compute_wt_overlapping(self, sequence: str) -> torch.Tensor:
        ...
        Enforces maximum chunk size of ANTIBERTY_MAX_AA_LEN (510 AA).
        ...
        # Enforce maximum window size for AntiBERTy
        max_window = ANTIBERTY_MAX_AA_LEN # 510 tokens
    ...
    def _compute_masked_optimal_batched(
        ...
        Enforces maximum chunk size of ANTIBERTY_MAX_AA_LEN (510 AA) + 2 special tokens = 512 tokens.
        ...
        # L is the number of actual residue tokens (excluding CLS and EOS)
        L = T - 2  # Subtract CLS and EOS
        V = self.vocab_size
...
def score_mutation_delta_logprob(
    ...
    *,
    ...
    for wt, pos, mt in iter_mutations(str(global_mut_str)):
        ...
        # Try to convert amino acids to token IDs
        # Use upper case for consistency
        wt_upper = wt.upper()
        mt_upper = mt.upper()

        wt_id = tokenizer.convert_tokens_to_ids(wt_upper)
        mt_id = tokenizer.convert_tokens_to_ids(mt_upper)

        # Handle cases where tokenizer returns string or list
        if isinstance(wt_id, (list, tuple)):
            wt_id = wt_id[0] if wt_id else tokenizer.unk_token_id
        if isinstance(mt_id, (list, tuple)):
            mt_id = mt_id[0] if mt_id else tokenizer.unk_token_id
        ...
        V = lprobs.size(1)
        if wt_id < 0 or wt_id >= V or mt_id < 0 or mt_id >= V:
            prefix = f"[{context}] " if context else ""
            # Check if it's an unknown token issue
            unk_id = tokenizer.unk_token_id if hasattr(tokenizer, 'unk_token_id') else None
            error_msg = (
                f"[Input file: {input_csv or 'unknown'}] "
                f"{prefix}Token ID out of bounds at position {pos}. "
                f"wt={wt} (upper={wt_upper}, id={wt_id}), "
                f"mt={mt} (upper={mt_upper}, id={mt_id}), "
                f"vocab_size={V}, unk_token_id={unk_id}"
            )
...
def main() -> None:
    ...
    parser.add_argument(
        "--model-path",
        required=True,
        help="Local path to AntiBERTy model",
    )
    parser.add_argument(
        "--input-csv",
        required=True,
    ...
    for group in tqdm(processor, desc="Scoring POI groups"):
        ...
        # Process chains separately for AntiBERTy
        log_probs = engine.get_log_probs_per_chain(
            group.chain_sequences,
            group.chain_offsets,
            args.mode
        )
    ...
    df[f"antiberty_{args.mode}_score"] = final_scores
