#!/usr/bin/env bash
# SabDab special-model batch training script
# - Uses the cluster-based split generated by prepare_SabDab_split_by_cluster.py
# - Writes outputs to results/SabDab using the same directory layout as standard models
# - The special models can run in parallel on different GPUs
...
GPUS="${GPUS:-0,1,2,3}"           # Default to GPUs 0-3
GPUS="${GPUS// /}"                # Remove whitespace
FOLDS="${FOLDS:-5}"               # SabDab uses 5-fold cross-validation
EPOCHS="${EPOCHS:-200}"
...
echo "===================================================================="
echo "SabDab Special-Model Batch Training (Dunbar2014)"
echo "===================================================================="
...
if (( GPU_COUNT >= 5 )); then
  # If GPU_COUNT >= 5, assign a dedicated GPU to each model.
  ...
  # If GPU_COUNT >= 3, distribute GPUs as evenly as possible.
  GPU_ABLANG2="${GPU_ARRAY[0]}"
  GPU_ANTIBERTY="${GPU_ARRAY[1]}"
  GPU_IGBERT="${GPU_ARRAY[2]}"
  ...
  # If only two GPUs are available, alternate assignments.
  GPU_ABLANG2="${GPU_ARRAY[0]}"
  GPU_ANTIBERTY="${GPU_ARRAY[1]}"
  ...
  # If only one GPU or no GPU is available, all models share the same setting.
  GPU_ABLANG2="$GPUS"
  GPU_ANTIBERTY="$GPUS"
  GPU_IGBERT="$GPUS"
  GPU_ONEHOT="$GPUS"
...
run_one() {
  ...
  LOG_FILE="$LOG_DIR/${STAMP}_sabdab_${SPLIT_STRATEGY}_${SAFE_MODEL}.log"

  echo "----------------------------------------------------------------"
  echo "    Train: $MODEL_KEY"
  echo "    Mode: $SPLIT_STRATEGY"
  echo "    GPU:  $GPU_ID"
  echo "    Log:  $LOG_FILE"
  echo "----------------------------------------------------------------"
...
# Single-model mode (enabled via the SINGLE_MODEL environment variable)
if [[ -n "${SINGLE_MODEL:-}" ]]; then
  echo "Single-model training mode: $SINGLE_MODEL"
  GPU_ID="${SINGLE_GPU:-$GPUS}"
  run_one "$SINGLE_MODEL" "$GPU_ID" || exit "$?"
else
  FAILURES=0
  echo "Batch training mode (${#MODELS[@]} special models)"
  echo

  # Launch the selected models in parallel.
  declare -a PIDS=()
...
echo
echo "===================================================================="
echo "SabDab Special-Model Training Complete"
echo "===================================================================="
