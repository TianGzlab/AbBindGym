#!/usr/bin/env bash
# Batch training for SabDab supervised models.
...
DATA_PATH="${DATA_PATH:-$REPO_ROOT/data/supervised/clustered_benchmarks/SabDab/csv/SabDab_with_clusters.csv}"
...
SPLIT_PREFIX="${SPLIT_PREFIX:-$REPO_ROOT/data/supervised/clustered_benchmarks/SabDab/splits/Dunbar2014_SabDab_seqcluster}"
SPLITS_FILE="${SPLITS_FILE:-${SPLIT_PREFIX}_k${FOLDS}_seed${SEED}.json}"

SUMMARY_CSV="$LOG_DIR/batch_train_sabdab.csv"
if [[ ! -f "$SUMMARY_CSV" ]]; then
  echo "timestamp,model,net,seed,status,log_path,splits_file,data_path" > "$SUMMARY_CSV"
fi

COMPLETED_SUMMARY="${COMPLETED_SUMMARY:-$REPO_ROOT/results/supervised/SabDab/csv/SabDab_with_clusters_model_summary.csv}"
...
if ! sabdab_validate_split_assets "$PYTHON_BIN" "$DATA_PATH" "$SPLITS_FILE" "$FOLDS"; then
  echo "Invalid SabDab split definition: $SPLITS_FILE"
  exit 1
fi

echo "===================================================================="
echo "SabDab Batch Training"
echo "===================================================================="
...
run_one() {
  ...
  SAFE_NET="${NET_NAME//\//_}"
  LOG_FILE="$LOG_DIR/${STAMP}_sabdab_${SAFE_MODEL}__${SAFE_NET}.log"
  ...
  local launcher=()
  if [[ "$USE_DDP" = "1" && $GPU_COUNT -gt 1 ]]; then
    if sabdab_command_exists "$TORCHRUN_BIN"; then
...
echo "===================================================================="
echo "SabDab Batch Training Complete"
echo "===================================================================="
