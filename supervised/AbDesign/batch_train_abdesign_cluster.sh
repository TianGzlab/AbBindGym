...
SEED="${SEED:-314}"
abdesign_resolve_profile cluster "$SEED"
...
SUMMARY_CSV="$LOG_DIR/batch_train_abdesign_cluster.csv"
...
run_one() {
  ...
  safe_net="${net_name//\//_}"
  log_file="$LOG_DIR/${stamp}_abdesign_cluster_${safe_model}__${safe_net}.log"
  ...
  cmd+=(
    ...
    --run-tag "${stamp}_abdesign_cluster"
  )
