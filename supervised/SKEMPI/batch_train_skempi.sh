#!/usr/bin/env bash
# Batch training for SKEMPI supervised models.
...
SUMMARY_CSV="$LOG_DIR/batch_train_skempi.csv"
...
run_one() {
  ...
  SAFE_NET="${NET_NAME//\//_}"
  LOG_FILE="$LOG_DIR/${STAMP}_skempi_${SAFE_MODEL}__${SAFE_NET}.log"
  ...
  local launcher=()
  if [[ "$USE_DDP" = "1" && $GPU_COUNT -gt 1 ]]; then
    if skempi_command_exists "$TORCHRUN_BIN"; then
...
echo "===================================================================="
echo "SKEMPI Batch Training Complete"
echo "===================================================================="
