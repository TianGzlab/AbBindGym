...
bindinggym_resolve_profile() {
  local seed="${1:-314}"
  local helper_dir
  helper_dir="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
  local repo_root="${BINDINGGYM_REPO_ROOT:-$(cd "$helper_dir/../.." && pwd)}"

  BINDINGGYM_PROFILE_NAME="full"
  BINDINGGYM_DATA_PATH="$repo_root/data/supervised/clustered_benchmarks/BindingGYM/csv/BindingGYM.csv"
  BINDINGGYM_SPLITS_FILE="$repo_root/data/supervised/clustered_benchmarks/BindingGYM/splits/BindingGYM_random_k5_seed${seed}.json"
  BINDINGGYM_RESULTS_ROOT="$repo_root/results/supervised/BindingGYM"
  BINDINGGYM_FOLDS=5
  BINDINGGYM_DATASET_NAME="BindingGYM"

  export BINDINGGYM_PROFILE_NAME
  export BINDINGGYM_DATA_PATH
  export BINDINGGYM_SPLITS_FILE
  export BINDINGGYM_RESULTS_ROOT
  export BINDINGGYM_FOLDS
  export BINDINGGYM_DATASET_NAME
}

bindinggym_validate_assets() {
  local data_path="$1"
  local splits_file="$2"
  local expected_folds="${3:-}"
  local python_bin="${BINDINGGYM_VALIDATION_PYTHON:-${PYTHON_BIN:-python3}}"
