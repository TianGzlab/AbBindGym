#!/usr/bin/env bash
# Batch training entrypoint for the PPB benchmark.
...
RESULTS_ROOT="${RESULTS_ROOT:-$REPO_ROOT/results/supervised/PPB}"
DATA_PATH="${DATA_PATH:-$REPO_ROOT/data/supervised/clustered_benchmarks/PPB/csv/PPB_with_clusters.csv}"
...
SPLIT_STRATEGY="${SPLIT_STRATEGY:-ab_ag_cluster}"
SPLITS_FILE="${SPLITS_FILE:-$REPO_ROOT/data/supervised/clustered_benchmarks/PPB/splits/PPB_k${FOLDS}_seed${SEED}.json}"
LOG_DIR="$RESULTS_ROOT/logs"
mkdir -p "$RESULTS_ROOT" "$MODEL_SAVE_DIR" "$LOG_DIR"

ppb_setup_hf_env

SUMMARY_CSV="$LOG_DIR/batch_train_ppb_${SPLIT_STRATEGY}.csv"
if [[ ! -f "$SUMMARY_CSV" ]]; then
  echo "timestamp,model,net,strategy,seed,status,log_path,splits_file,data_path" > "$SUMMARY_CSV"
fi

[[ -f "$TRAIN_SCRIPT" ]] || { echo "Training script not found: $TRAIN_SCRIPT"; exit 1; }
if ! ppb_validate_split_assets "$PYTHON_BIN" "$DATA_PATH" "$SPLITS_FILE" "$FOLDS"; then
  echo "Invalid PPB split definition: $SPLITS_FILE"
  exit 1
fi

echo "===================================================================="
echo "PPB Batch Training"
echo "===================================================================="
...
# #   # === Mid-size models (300M - 1B params) ===
...
# # #   # AIDO
 ...
 # ESM-1v
  ...
  # MAGE fine-tuned ProGen2 antibody generator
  "mage_progen2_epoch4_01|${MAGE_MODEL_DIR:-$HOME/.cache/huggingface/MAGE}/model_epoch4"
...
run_one() {
  ...
  SAFE_NET="${NET_NAME//\//_}"
  LOG_FILE="$LOG_DIR/${STAMP}_ppb_${SPLIT_STRATEGY}_${SAFE_MODEL}__${SAFE_NET}.log"
  ...
  local launcher=()
  if [[ "$USE_DDP" = "1" && $GPU_COUNT -gt 1 ]]; then
    if ppb_command_exists "$TORCHRUN_BIN"; then
  ...
  echo "$(date +%F\ %T),$MODEL_NAME,$NET_NAME,$SPLIT_STRATEGY,$SEED,$STATUS,$LOG_FILE,$SPLITS_FILE,$DATA_PATH" >> "$SUMMARY_CSV"
...
echo "===================================================================="
echo "PPB Batch Training Complete"
echo "===================================================================="
